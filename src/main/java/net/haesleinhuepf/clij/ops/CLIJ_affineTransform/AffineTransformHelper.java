...
public class AffineTransformHelper {
	...
	public static float[] transformStringToMatrix(String transform,
		float inputWidth, float inputHeight, float inputDepth)
	{
		...
		for (String transformCommand : transformCommands) {
			...
			if (commandParts[0].compareTo("center") == 0) {
				...
				float angle = (float) (-Float.valueOf(commandParts[1]) / 180.0f *
					Math.PI);
				at.rotate(0, angle);
			}
			...
			else if (commandParts[0].compareTo("rotatez") == 0 || commandParts[0]
				.compareTo("rotate") == 0)
			{
				float angle = (float) (-Float.valueOf(commandParts[1]) / 180.0f *
					Math.PI);
		...
		return net.haesleinhuepf.clij.utilities.AffineTransform.matrixToFloatArray(
			at);
	}
